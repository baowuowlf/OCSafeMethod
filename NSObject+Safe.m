...
#define SFAssert(condition, ...) \
    if (!(condition)) { SFLog(__FILENAME__, __FUNCTION__, __LINE__, __VA_ARGS__); } \
    NSAssert(condition, @"%@", __VA_ARGS__);

void SFLog(const char* file, const char* func, int line, NSString* fmt, ...){
    va_list args; va_start(args, fmt);
    NSLog(@"\033[fg255,0,0;%s:%d %s> %@\033[;", file, line, func, [[NSString alloc] initWithFormat:fmt arguments:args] );
    va_end(args);
}
...
- (void)nos_removeObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath {
    ...
    @try {
        ...
        NSLog(@"nos_removeObserver ex: %@", [exception callStackSymbols]);
    }
}
